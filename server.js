...
const RATE_LIMIT = 100; // drawing needed high limit for events per second as it was not registering the strokes
...
const rooms = new Map();
const pendingRoomDeletions = new Map(); // 2min period before deleting empty rooms
...
class GameRoom {
  ...
  broadcastState() {
    ...
    // Determine if we should reveal the word to everyone
    // During ROUND_OVER or GAME_OVER, everyone should see the word
    const shouldRevealWord = this.phase === 'ROUND_OVER' || this.phase === 'GAME_OVER';
    
    // Send to each player individually - drawer gets the word during DRAWING, everyone during ROUND_OVER
    this.players.forEach(player => {
      ...
      if (socket) {
        socket.emit('game_event', {
          ...
          payload: {
            ...baseState,
            // SECURITY: Only send currentWord to drawer during DRAWING phase
            // Everyone gets the word during ROUND_OVER/GAME_OVER for reveal
            currentWord: (shouldRevealWord || player.id === drawer?.id) ? this.currentWord : ''
          }
        });
      }
    });
  ...
  // picks 3 words, always includes 1 hard one cuz why not right?
  getWordOptions() {
